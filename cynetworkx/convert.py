...
graph constructor.  The constructor calls the to_cynetworkx_graph() function
...
def _prep_create_using(create_using):
    ...
    If create_using is None return the default (just cynetworkx.Graph())
    If create_using.clear() works, assume it returns a graph object.
    Otherwise raise an exception because create_using is not a cynetworkx graph.
    ...
    try:
        ...
        raise TypeError("Input graph is not a cynetworkx graph type")
    return create_using


def to_cynetworkx_graph(data, create_using=None, multigraph_input=False):
