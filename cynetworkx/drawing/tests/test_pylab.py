...
class TestPylab(object):
    ...
    def test_labels_and_colors(self):
        ...
        # nodes
        nx.draw_cynetworkx_nodes(G, pos,
                               nodelist=[0, 1, 2, 3],
                               node_color='r',
                               node_size=500,
                               alpha=0.8)
        nx.draw_cynetworkx_nodes(G, pos,
                               ...
                               alpha=0.8)
        # edges
        nx.draw_cynetworkx_edges(G, pos, width=1.0, alpha=0.5)
        nx.draw_cynetworkx_edges(G, pos,
        ...
        labels[6] = r'$\gamma$'
        labels[7] = r'$\delta$'
        nx.draw_cynetworkx_labels(G, pos, labels, font_size=16)
        plt.show()
    ...
    def test_alpha_iter(self):
        ...
        plt.subplot(131)
        nx.draw_cynetworkx_nodes(self.G, pos, alpha=[0.1, 0.2])
        # with equal alpha elements and nodes
        num_nodes = len(self.G.nodes)
        alpha = [x / num_nodes for x in range(num_nodes)]
        colors = range(num_nodes)
        plt.subplot(132)
        nx.draw_cynetworkx_nodes(self.G, pos, node_color=colors, alpha=alpha)
        # with more alpha elements than nodes
        alpha.append(1)
        plt.subplot(133)
        nx.draw_cynetworkx_nodes(self.G, pos, alpha=alpha)
