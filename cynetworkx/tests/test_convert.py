...
from cynetworkx.testing import assert_nodes_equal, assert_edges_equal, assert_graphs_equal
from cynetworkx.convert import (to_cynetworkx_graph,
                              to_dict_of_dicts,
                              from_dict_of_dicts,
                              to_dict_of_lists,
                              from_dict_of_lists)
from cynetworkx.generators.classic import barbell_graph, cycle_graph
...
class TestConvert():
    ...
    def test_exceptions(self):
        # _prep_create_using
        G = {"a": "a"}
        H = nx.to_cynetworkx_graph(G)
        assert_graphs_equal(H, nx.Graph([('a', 'a')]))
        assert_raises(TypeError, to_cynetworkx_graph, G, create_using=0.0)

        # NX graph
        class G(object):
            adj = None

        assert_raises(nx.NetworkXError, to_cynetworkx_graph, G)
        ...
        # Dict of [dicts, lists]
        G = {"a": 0}
        assert_raises(TypeError, to_cynetworkx_graph, G)
        ...
        # no match
        assert_raises(nx.NetworkXError, to_cynetworkx_graph, "a")
    ...
    def test_digraphs(self):
        for dest, source in [(to_dict_of_dicts, from_dict_of_dicts),
            ...
            GW = to_cynetworkx_graph(dod, create_using=nx.DiGraph())
    ...
    def test_graph(self):
        ...
        assert_nodes_equal(sorted(G.nodes()), sorted(GG.nodes()))
        assert_edges_equal(sorted(G.edges()), sorted(GG.edges()))
        GW = to_cynetworkx_graph(dod, create_using=nx.Graph())
        assert_nodes_equal(sorted(G.nodes()), sorted(GW.nodes()))
        assert_edges_equal(sorted(G.edges()), sorted(GW.edges()))
        GI = nx.Graph(dod)
        assert_equal(sorted(G.nodes()), sorted(GI.nodes()))
        ...
        GW = to_cynetworkx_graph(dol, create_using=nx.Graph())
        assert_nodes_equal(sorted(G.nodes()), sorted(GW.nodes()))
    ...
    def test_with_multiedges_self_loops(self):
        ...
        GW = to_cynetworkx_graph(dod, create_using=nx.MultiGraph(), multigraph_input=True)
        ...
        assert_nodes_equal(sorted(G.nodes()), sorted(GG.nodes()))
        assert_edges_equal(sorted(G.edges()), sorted(GG.edges()))
        GW = to_cynetworkx_graph(dol, create_using=nx.MultiGraph())
        assert_nodes_equal(sorted(G.nodes()), sorted(GW.nodes()))
        assert_edges_equal(sorted(G.edges()), sorted(GW.edges()))
    ...
    def test_attribute_dict_integrity(self):
        ...
        H = to_cynetworkx_graph(G, create_using=nx.OrderedGraph())
