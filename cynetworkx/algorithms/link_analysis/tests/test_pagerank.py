...
class TestPageRank(object):
    ...
    def test_pagerank(self):
        G = self.G
        p = cynetworkx.pagerank(G, alpha=0.9, tol=1.e-08)
        for n in G:
            assert_almost_equal(p[n], G.pagerank[n], places=4)

        nstart = dict((n, random.random()) for n in G)
        p = cynetworkx.pagerank(G, alpha=0.9, tol=1.e-08, nstart=nstart)
        for n in G:
            assert_almost_equal(p[n], G.pagerank[n], places=4)

    @raises(cynetworkx.PowerIterationFailedConvergence)
    def test_pagerank_max_iter(self):
        cynetworkx.pagerank(self.G, max_iter=0)

    def test_numpy_pagerank(self):
        G = self.G
        p = cynetworkx.pagerank_numpy(G, alpha=0.9)
        for n in G:
            assert_almost_equal(p[n], G.pagerank[n], places=4)
        personalize = dict((n, random.random()) for n in G)
        p = cynetworkx.pagerank_numpy(G, alpha=0.9, personalization=personalize)

    def test_google_matrix(self):
        G = self.G
        M = cynetworkx.google_matrix(G, alpha=0.9, nodelist=sorted(G))
    ...
    def test_zero_personalization_vector(self):
        ...
        assert_raises(ZeroDivisionError, cynetworkx.pagerank, G,
    ...
    def test_dangling_matrix(self):
        ...
        M1 = cynetworkx.google_matrix(G, personalization=dangling)
        M2 = cynetworkx.google_matrix(G, personalization=dangling,
                                    dangling=dangling)
    ...
    def test_dangling_pagerank(self):
        pr = cynetworkx.pagerank(self.G, dangling=self.dangling_edges)
        for n in self.G:
            assert_almost_equal(pr[n], self.G.dangling_pagerank[n], places=4)

    def test_dangling_numpy_pagerank(self):
        pr = cynetworkx.pagerank_numpy(self.G, dangling=self.dangling_edges)
        for n in self.G:
            assert_almost_equal(pr[n], self.G.dangling_pagerank[n], places=4)

    def test_empty(self):
        G = cynetworkx.Graph()
        assert_equal(cynetworkx.pagerank(G), {})
        assert_equal(cynetworkx.pagerank_numpy(G), {})
        assert_equal(cynetworkx.google_matrix(G).shape, (0, 0))
...
class TestPageRankScipy(TestPageRank):
    ...
    def test_scipy_pagerank(self):
        G = self.G
        p = cynetworkx.pagerank_scipy(G, alpha=0.9, tol=1.e-08)
        for n in G:
            assert_almost_equal(p[n], G.pagerank[n], places=4)
        personalize = dict((n, random.random()) for n in G)
        p = cynetworkx.pagerank_scipy(G, alpha=0.9, tol=1.e-08,
    ...
    def test_scipy_pagerank_max_iter(self):
        cynetworkx.pagerank_scipy(self.G, max_iter=0)

    def test_dangling_scipy_pagerank(self):
        pr = cynetworkx.pagerank_scipy(self.G, dangling=self.dangling_edges)
        for n in self.G:
            assert_almost_equal(pr[n], self.G.dangling_pagerank[n], places=4)

    def test_empty_scipy(self):
        G = cynetworkx.Graph()
        assert_equal(cynetworkx.pagerank_scipy(G), {})
