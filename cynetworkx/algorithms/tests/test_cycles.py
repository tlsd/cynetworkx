...
from cynetworkx.algorithms import find_cycle
from cynetworkx.algorithms import minimum_cycle_basis
...
class TestCycles:
    ...
    def test_cycle_basis(self):
        G = self.G
        cy = cynetworkx.cycle_basis(G, 0)
        sort_cy = sorted(sorted(c) for c in cy)
        assert_equal(sort_cy, [[0, 1, 2, 3], [0, 1, 6, 7, 8], [0, 3, 4, 5]])
        cy = cynetworkx.cycle_basis(G, 1)
        sort_cy = sorted(sorted(c) for c in cy)
        assert_equal(sort_cy, [[0, 1, 2, 3], [0, 1, 6, 7, 8], [0, 3, 4, 5]])
        cy = cynetworkx.cycle_basis(G, 9)
        sort_cy = sorted(sorted(c) for c in cy)
        assert_equal(sort_cy, [[0, 1, 2, 3], [0, 1, 6, 7, 8], [0, 3, 4, 5]])
...
class TestFindCycle(object):
    ...
    def test_prev_explored(self):
        # https://github.com/cynetworkx/cynetworkx/issues/2323
    ...
    def test_no_cycle(self):
        # https://github.com/cynetworkx/cynetworkx/issues/2439
