...
def extrema_bounding(G, compute="diameter"):
    ...
    while candidates:
        ...
        dist = dict(cynetworkx.single_source_shortest_path_length(G, current))
        if len(dist) != N:
            msg = ('Cannot compute metric because graph is not connected.')
            raise cynetworkx.NetworkXError(msg)
...
#        nodes=v
    ...
    for n in G.nbunch_iter(v):
        if sp is None:
            length = cynetworkx.single_source_shortest_path_length(G, n)
            L = len(length)
        else:
            try:
                length = sp[n]
                ...
                raise cynetworkx.NetworkXError('Format of "sp" is invalid.')
        if L != order:
            if G.is_directed():
